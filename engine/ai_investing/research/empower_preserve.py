...
BRAKE_DD = 0.06          # flat when equity is 6% under its rolling peak
GATE_SMA = 100           # universe EW index vs its 100-day average
GATE_REARM = 10          # consecutive green days required to resume

def run(O, L, C, symbols, buy_sig, sell_sig, trig_low):
    ew = (C / C.iloc[0]).mean(axis=1)
    gate_ok = (ew > ew.rolling(GATE_SMA).mean()).fillna(False)
    gate_exit = (ew < ew.rolling(GATE_SMA).mean() * 0.98).fillna(False)
    ...
    breaker = False
    green_streak = 0
    YEAR_BUDGET = 0.05       # hard annual loss budget: -5% YTD -> flat till Jan 1
    year_start_eq = START_CASH
    cur_year = C.index[35].year
    halted_year = False
    for i in range(35, len(C)):
        ...
        if C.index[i].year != cur_year:
            ...
            halted_year = False
        # market-gate / circuit-breaker / year-budget liquidation at the open
        force_flat = breaker or halted_year or gate_exit.iloc[i - 1]
        for sym in list(pos):
            v = op[sym]
            if np.isnan(v):
                continue
        ...
        # circuit breaker: 6% under rolling peak -> flat until gate green 10d
        peak = max(peak, eq)
        if not breaker and eq <= peak * (1 - BRAKE_DD):
            breaker = True
            green_streak = 0
        if breaker:
            green_streak = green_streak + 1 if gate_ok.iloc[i] else 0
            if green_streak >= GATE_REARM:
                breaker = False
                peak = eq                             # fresh baseline on resume
        if not halted_year and eq <= year_start_eq * (1 - YEAR_BUDGET):
            halted_year = True                        # year's risk budget spent
        allow_entries = (not breaker) and (not halted_year) and gate_ok.iloc[i - 1]
        for sym in symbols:
            if not allow_entries or len(pos) >= SLOTS:
                break
...
for period in ("3y", "10y"):
    ...
    cagr, sh, dd = stats(c)
    print(f"\n=== PRESERVATION EXPERT (gate+breaker) — {period} "
