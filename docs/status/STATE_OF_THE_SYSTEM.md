...
| ~~The 258-symbol watchlist is pushing one LLM endpoint toward its free daily cap~~ (§4.25) | **CLOSED 2026-09-18 — it stopped being a projection and became a bill, and the guard could not stop it.** Every endpoint crossed 90% and `ep-...vgxfw` finished at **122.7%, 1.16M tokens past a 5M/day allowance**, driven by the crypto-wave backfill burning ~2.9M tokens/hour against a live-loop baseline of ~130k/hour. The cause was structural, not a burst: `_call_byteplus_chain` fell back to `(spare or chain)` — the FULL chain, whose head is the endpoint that accumulated FIRST, i.e. the most spent — so the guard meant to rotate AWAY from a spent endpoint routed straight back into it the moment every endpoint crossed 90%. Rotation was a preference with no cap behind it. `_at_free_cap()` is now a real gate: it filters every candidate (`spare` only ORDERS the result — letting the deliberately fail-open rotation check filter would re-admit the paid call), the call's own worst case is reserved because the meter is read before a call and written after it, and an unreadable meter fails CLOSED while an ABSENT one does not (absence only means nothing has been spent yet). Refusals are counted in `llm_usage.json`, logged hourly, and reported as their own `daily_status.py` row for the watchdog to page on, because a brain that stopped reading is a degradation and must not be silent. BytePlus is the only cloud LLM path on the ProDesk (no Anthropic/DeepSeek key is set), so this caps the whole exposure. `engine/tests/test_llm_hard_cap.py`. | The cap trades spend for perception: once every endpoint is at its allowance the tagger runs on the keyword floor until 00:00 UTC. That is deliberate and now visible (the new refusals row), but it is a real degradation to weigh against raising `LLM_DAILY_FREE_TOKENS`. **Still unverified:** whether BytePlus bills the overage or merely refuses it. Calls at 122.7% were still succeeding, which means either the grant is larger than the 5M this project recorded in `68025cf`, or the excess was charged — only the provider console distinguishes them. |
...
| **The curated tier has never digested a real piece** (§4.60) | **The first file the operator drops into `data/curated/`.** Everything about the tier is tested, mutation-verified and deployed; nothing has gone through it end to end with a live extractor. What the tests CANNOT cover is the part that needs a real LLM: whether the extractor actually returns a rich `proposed_edges` list off a long-form piece, or keeps behaving as though it were reading a wire item and returns one edge. If it returns 0–1 edges from a genuinely mechanism-dense piece, the bottleneck moved to the PROMPT (§A12's `proposed_edges` block) and the tier's plumbing is not the thing to change. **Negative answer written down in advance, per §4.52: `user_edges: 0` after a piece was digested means the extraction failed, not that the piece was thin.** | `brain_audit.py --section graph` → `user_edges`, `user_nodes`, `user_sources`. Cross-check the piece was seen at all — this separates "never ingested" from "ingested, extracted nothing", which are different failures: `sqlite3 data/brain.db "select title, source, digested from articles where source like 'user_curated%' order by rowid desc limit 5"`. |
| **The NN shadow book's first gradable rows** | **2026-08-27**, five days after the lane went live on 2026-08-22 (275 primary decisions on its first cycle). Until then the report is all `pending`, and that is not a failure. **Read `n_independent`, never `graded`** — ~275 rows/day is at best ~55 independent observations and they are cross-sectionally correlated on top of that, so the honest figure is smaller again. **Negative answer written in advance:** the net disagreed with the brain on **157 of 275** calls (57%) and sized ~7× larger on the row inspected, which is what an over-fitted 49-parameter model looks like *and also* what a genuinely better model would look like. `missed` vs `wrong` is what separates them — a high `wrong` count means reckless, a high `missed` count on the brain's side means the brain is too timid. Do not read either as a verdict before `n_independent` is well past 20. | `scripts/nn_shadow_report.py` |
...
| ~~LLM endpoint nearing its free daily cap~~ | **CUE DISCHARGED 2026-09-18, and it fired the other way.** Not "3 consecutive days above 90%" but a single day past 100% with all three endpoints spent, so the choice this cue was holding — "trim per-cycle scoring frequency or add a second paid endpoint" — is no longer the only two options: `_at_free_cap()` now refuses the call instead of billing it. What is left is a judgement call rather than a bug: run on the keyword floor once the allowance is gone, or raise `LLM_DAILY_FREE_TOKENS`. | `daily_status.py` → "LLM budget refusals" (the new row) and "LLM free allowance". |
