...
| **No position has a venue stop — and that is now mostly by design, not the 2026-08-05 mystery** | Re-checked 2026-08-21: the account's resting orders are LO/MO only, **zero protective orders**, across all 13 positions. Most of that is deliberate — `SHARED_STOCK_ACCOUNT` skips venue-side stops because a stop firing hours later cannot be attributed to a book (see `design/SHARED_ACCOUNT.md`). The original entry was about one unexplained `place_stop` failure for AAPL on 2026-08-05; that specific mystery is now moot, because the shared-account design superseded the code path. The REAL open item is the consequence, which the old framing understated. | Every position relies on the engine's cycle stop, which only fires when a cycle runs — precisely what an overnight gap defeats. This was bounded when it was one $307 share; it now covers ~$4,800 across ten names in the trading book plus the sleeve's three. Not a bug to patch: it is the price of the shared account, and the mitigation is either per-book sub-accounts or accepting gap risk explicitly. |
...
| CRWV avoid/short call | The `ai_circularity → crwv` edge needs **n≥20** realised-return observations following an `ai_circularity` activation before `calibration.py` can issue a verdict (`MIN_N = 20`) — currently below that. Re-run the calibrator periodically; a "contradicted" verdict there would be the trigger to demote the edge the same way TAO/FET's `crypto_majors` membership already was. | `python3 -m ai_investing.brain.calibration`, look for an `ai_circularity`→`crwv` row. |
| `UNI/USD`'s override (now zeroed in code) | Remove `CONFIRMED_MISCALIBRATED` once **both** hold: (a) UNI has a real graph node, and (b) `calibration.py` has scored its edges at n≥20. Removing it before either is true would just re-admit the same miscalibrated formula-only score. | `brain/adviser.py`, `data/knowledge_graph.json` for a `uni`/`uniswap` node, then the calibration report. |
