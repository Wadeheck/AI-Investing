...
| Adviser predicts well; books don't trade it | Judgement call, not a bug — but quantifiable: if the adviser's long-side hit-rate holds **>0.60** and the formula-engine's short/avoid side holds **<0.35** across **n≥500** each for **30+ calendar days**, that is the trigger to reconsider wiring adviser conviction into position sizing rather than treating it as advisory-only. | `advice_outcomes` vs `journal.db decisions`, same query as SCORECARD_REVIEW §3. |
